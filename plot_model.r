...
image((min(caData$x):max(caData$x)), (min(caData$y):max(caData$y)), zData, xlab='', ylab='', xaxt='n', yaxt='n', xlim=range(xTickLocs), ylim=range(yTickLocs), col=colors)
...
legend(min(caData$x), 1.05*max(caData$y), legend=c('R', 'T', 'B', 'M'), fill=colors, horiz=TRUE, bty='n')
dev.off()